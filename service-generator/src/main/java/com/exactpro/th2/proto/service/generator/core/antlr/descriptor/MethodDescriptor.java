...
public class MethodDescriptor extends CommentableDescriptor {
    ...
    public static MethodDescriptor newInstance(MethodDescriptor md) {
        var requestTypes = md.getRequestTypes().stream()
                .map(TypeDescriptor::newInstance)
                .collect(Collectors.toList());

        return MethodDescriptor.builder()
                .comments(new ArrayList<>(md.getComments()))
                .name(md.getName())
                .responseType(TypeDescriptor.newInstance(md.getResponseType()))
